...
class OnlineEvalProducer(DaemonTask):
    ...
    def __init__(
        ...
        *,
        ...
        # across all projects and tenants — 100k ids per 3600s sweep (the defaults)
        # only protects deployments ingesting under ~28 spans/sec instance-wide.
        # Above that, rows can age out of the lookback between sweeps and the
        # backstop stops bounding the exceptional loss modes it exists to catch
        # (late-visible spans, skipped criteria). The reaper's floor (``_reap``) is
        # aligned to this lookback, so any change here must move both together.
